...
2. Bump the version (updates `manifest.json` + `versions.json` via
   `version-bump.mjs`):
   ```bash
...
3. Push the commit and the tag:
   ...
   ```
4. `release.yml` builds and creates a GitHub Release with `main.js`,
...
- **On-device QA.** Run `QA.md` on device before a public release; check
  `KNOWN-ISSUES.md`.
